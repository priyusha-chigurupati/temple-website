...
try {
    $upsertSetting($connection, (int) $localeId, 'seo.title_suffix', 'AnkammaThalli Temple', 'string', true);
    $upsertSetting($connection, (int) $localeId, 'seo.default_description', 'Explore temple heritage, sacred events, gallery archives, and ways to support the sanctuary.', 'text', true);

    $connection->commit();

    echo 'SEO settings seeded successfully.' . PHP_EOL;
} catch (Throwable $exception) {
    if ($connection->inTransaction()) {
        $connection->rollBack();
    }

    fwrite(STDERR, 'SEO settings seed failed: ' . $exception->getMessage() . PHP_EOL);
    exit(1);
}
