...
$navigation = is_array($content['navigation'] ?? null) ? $content['navigation'] : [];
...
try {
    $upsertSetting($connection, (int) $localeId, 'header.primary_cta.label', 'Donate Now', 'string', true);
    $upsertSetting($connection, (int) $localeId, 'header.primary_cta.href', '/donations', 'string', false);
    $upsertSetting($connection, (int) $localeId, 'header.navigation_items', $navigation, 'json', true);

    $connection->commit();

    echo 'Header settings seeded successfully.' . PHP_EOL;
} catch (Throwable $exception) {
    if ($connection->inTransaction()) {
        $connection->rollBack();
    }

    fwrite(STDERR, 'Header settings seed failed: ' . $exception->getMessage() . PHP_EOL);
    exit(1);
}
